...
class UserController {
  static async register(ctx, next) {
    ...
    if (account) {
      // 查询用户名是否重复
      const existUser = await userModel.findUser({account})
      if (existUser && existUser.length !== 0) {
        ...
        let newUser = await userModel.findUser({account})
  ...
  static async getUserInfo(ctx, next) {
    let userInfo = await userModel.getUserInfo({account: ctx.state.user.account})
    if (userInfo) {
